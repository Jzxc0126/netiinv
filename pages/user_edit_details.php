...
while ($row = mysqli_fetch_array($result2)) {
  ...
  $dprtmntid = $row['u_departmentid'];
  $dprtmnt = $row['department'];
  $level = $row['level'];
  $levelid = $row['u_levelid'];
...
<div class="container-fluid mt-3" style="max-width:800px;">
  <div class="card" style="background-color: #ffffffba;background-clip: border-box;border-radius: 2.5rem;margin-bottom: 10px;-webkit-box-shadow: 1px 1px 23px 9px rgba(0,0,0,0.75);-moz-box-shadow: 1px 1px 23px 9px rgba(0,0,0,0.75);box-shadow: 1px 1px 23px 9px rgb(0 71 171 / 55%);">
    <div class="card-header py-3" style="background-color: #ffffffba;background-clip: border-box;border-radius: 2.5rem;margin-bottom: 5px;">
      <h4 class="m-2 font-weight-bold text-primary"><i class="fa-solid fa-user-tie"></i>&nbsp;Edit <?php echo $lname; ?>'s Details</h4>
    </div>
    <div class="row">
      <div class=col-md-12 style="margin-left: 20px;">
        <a href="users.php?" type="button" class="btn btn-primary bg-gradient-primary"><i class="fa-solid fa-arrow-left"></i></i> Back</a>
        
      </div>

    </div>
    <form action="" method="POST" onsubmit="return confirm('Are you sure you want to edit account?');">
      ...
      <div class="card-body">
        ...
        <div class="form-group row text-left">
          ...
          <div class="col-md-3">
            <h5>
              ...
              <option value="<?php echo $dprtmntid?>" style="background-color :rgb(1 105 250);color:white;"><?php echo $dprtmnt?></option>
        ...
        <div class="form-group row text-left">
          ...
          <div class="col-sm-3">
            <h5>
              :<select class="inputfield form-control" name="editselectacctype" id="editselectacctype" value="<?php echo $levelid ?>" >
              <option value="<?php echo $levelid?>" style="background-color :rgb(1 105 250);color:white;"><?php echo $level?></option>
              <?php populateselectacctype(); ?></select><br>
            </h5>
          </div>

        </div>
        <center>
        <button type="submit" class="btn btn-primary bg-gradient-success" name="btnupdateuser" value="Update" id="saveBtn">Update</button>
        </center>
...
if (isset($_POST["btnupdateuser"])) {
    ...
    $editacctype = $_POST["editselectacctype"];
    $userid = $u_ids;
    
    updateuseraccount($editdpt, $editacctype, $userid);
